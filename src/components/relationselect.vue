  ...
  <div class="relationSelect">
    ...
    <div class="relation-list">
      <table v-if="curValue.length>0" class="table is-narrow">
        ...
        <tbody>
          <tr v-for="(relation, index) in curValue" :key="index">
            ...
            <td class="has-text-right">
              <button type="button" class="button is-small is-inverted is-danger" @click.prevent="removeRelation(index)"><span class="icon is-small"><i class="fa fa-times"></i></span></button>
            </td>
          </tr>
        </tbody>
      </table>
        ...
        <p class="control">
          <span class="select">
            <select v-model="selRelation">
              ...
              <option v-for="(r, index) in allowedRelations" :value="r" :key="index">{{ r.charAt(0).toUpperCase() + r.slice(1) }}</option>
            </select>
          </span>
        </p>
        <p class="control">
          <div class="work-title">
            <autocomplete v-model.trim="txtWorkTitle" :suggestions="suggestions" placeholder="Titre" @knownvalue="workChange" />
          </div>
        </p>
        <p class="control">
          ...
          <button
            ...
            @click="addRelation(txtWorkTitle, selRelation)">
...
export default {
  ...
  computed: {
    ...
    suggestions() {
      return this.knownWorks
        .filter(w => !this.removeIds.includes(w.id))
        .reduce((works, work) => [...works, work.title], []);
    },
    ...mapGetters({
  ...
  methods: {
    addRelation(title, relation) {
      const work = this.workByTitle(title);
      ...
      if (work) {
        ...
        console.log(
          "ERREUR! Impossible de trouver l'oeuvre à mettre en relation!",
        );
      }
    },
